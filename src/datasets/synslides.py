...
INCLUDE_KEYWORDS = ("text", "title", "body", "bullet", "list", "caption",
                    "equation", "footer", "header", "heading", "paragraph")
EXCLUDE_KEYWORDS = ("image", "figure", "chart", "photo", "logo", "graphic",
                     "infographic", "icon", "diagram", "picture")
...
def iter_samples(raw_dir: str = RAW_DIR):
    ...
    for jp in coco_jsons:
        ...
        for img_id, im in images_by_id.items():
            ...
            yield Sample(sample_id=f"{tag}_{img_id}", image=img, polygons=polys, coarse=True)
